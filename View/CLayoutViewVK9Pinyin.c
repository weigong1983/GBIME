...
GBIMELOCAL GBIMEReturn CLayoutViewVK9Pinyin__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);

// 本地函数声明
GBIMELOCAL void ICLayoutViewVK9Pinyin__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);
GBIMELOCAL void ICLayoutViewVK9Pinyin__DrawAssociateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);
...
CLayoutViewAbstract * CLayoutViewVK9Pinyin__Construct(PCLayoutViewVK9Pinyin pltViewVK)
{
	...
	CLayoutViewVK9Pinyin__Init(pltViewVK);
	return (CLayoutViewAbstract *)pltViewVK;
}

GBIMEReturn CLayoutViewVK9Pinyin__Init(PCLayoutViewVK9Pinyin pltViewVK)
{
	pltViewVK->m_ltParent.m_ltParent.m_pfcPaint = CLayoutViewVK9Pinyin__Paint;
	return GBIME_OK;
}
...
GBIMELOCAL GBIMEReturn CLayoutViewVK9Pinyin__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	...
	CLASS_VIEW_THIS_POITER(CLayoutViewVK9Pinyin, pltThis)
	...
	if ((pltRoot->m_refreshFlag & GBIME_LAYOUT_REFRESH_CANDIDATE_LINE) == GBIME_LAYOUT_REFRESH_CANDIDATE_LINE)
	{
		...
		case FirstLayer:
			break;
		case SecondLayer:
			ICLayoutViewVK9Pinyin__DrawCandidateLine(pltObj, pEngineOutputInfo);
			break;
		case ThirdLayer:
			ICLayoutViewVK9Pinyin__DrawAssociateLine(pltObj, pEngineOutputInfo);
			break;
		default:
			PT_Assert(0);
			break;
...
*/
static void ICLayoutViewVK9Pinyin__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	CLASE_THIS_POITER(CLayoutVK9Pinyin, pltObj)
	CLASS_VIEW_THIS_POITER(CLayoutViewVK9Pinyin, pltThis)
	...
	// 打印Log信息
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************ICLayoutViewVK9Pinyin__DrawCandidateLine() Draw Candidate image\n"));
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************Candidate Image ID = %d\n", iCandidateNomalImageId));
	
	// 绘制拼音音节和候选字
	pltViewBase->m_pfcDrawPinYin(pltObj, pEngineOutputInfo);
...
*/
static void ICLayoutViewVK9Pinyin__DrawAssociateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	CLASE_THIS_POITER(CLayoutVK9Pinyin, pltObj)
	CLASS_VIEW_THIS_POITER(CLayoutViewVK9Pinyin, pltThis)
	...
	if (pLayerConstInfo != NULL)
	{
		...
		// 打印Log信息
		PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************ICLayoutViewVK9Pinyin__DrawAssociateLine() Draw Candidate image\n"));
		PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************Candidate Image ID = %d\n",iCandidateNomalImageId));
