...
GBIMELOCAL GBIMEReturn CLayoutViewKB9Pinyin__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);

// 本地函数声明
GBIMELOCAL void ICLayoutViewKB9Pinyin__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);
...
CLayoutViewAbstract * CLayoutViewKB9Pinyin__Construct(PCLayoutViewKB9Pinyin pltViewKB9)
{
	...
	CLayoutViewKB9Pinyin__Init(pltViewKB9);
	return (CLayoutViewAbstract *)pltViewKB9;
}

GBIMEReturn CLayoutViewKB9Pinyin__Init(PCLayoutViewKB9Pinyin pltViewKB9)
{
	pltViewKB9->m_ltParent.m_ltParent.m_pfcPaint = CLayoutViewKB9Pinyin__Paint;
	return GBIME_OK;
}
...
GBIMELOCAL GBIMEReturn CLayoutViewKB9Pinyin__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
 	...
 	CLASS_VIEW_THIS_POITER(CLayoutViewKB9Pinyin, pltThis)
...
// 	}
		...
		&& pEngineOutputInfo->bCandWindOpen)
	{
		...
		{
		case FirstLayer:
			ICLayoutViewKB9Pinyin__DrawCandidateLine(pltObj, pEngineOutputInfo);
			break;
		default:
			PT_Assert(0);
			break;
...
static void ICLayoutViewKB9Pinyin__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	CLASE_THIS_POITER(CLayoutKB9Pinyin, pltObj)
	CLASS_VIEW_THIS_POITER(CLayoutViewKB9Pinyin, pltThis)
...
// 	PT_DrawImage(iDisplayImageLeftX ,iDisplayImageTopY,iCandidateNomalImageId);
	...
	// 打印Log信息
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************ICLayoutViewKB9Pinyin__DrawCandidateLine() Draw Candidate image\n"));
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************Candidate Image ID = %d\n", iCandidateNomalImageId));
	
	// 绘制拼音音节和候选字
	if(!pEngineOutputInfo->nSyllableNum)
	{
