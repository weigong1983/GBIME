/***************************************************************************
* File Name    :  CLayoutViewVK26MultiLang.c
...
GBIMELOCAL GBIMEReturn CLayoutViewVK26MultiLang__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);

// 本地函数声明
GBIMELOCAL void ICLayoutViewVK26MultiLang__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);

CLayoutViewAbstract * CLayoutViewVK26MultiLang__Construct(PCLayoutViewVK26MultiLang pltViewVK26)
{
	CLayoutViewVK__Construct(&pltViewVK26->m_ltParent);
	CLayoutViewVK26MultiLang__Init(pltViewVK26);
	return (CLayoutViewAbstract *)pltViewVK26;
}

GBIMEReturn CLayoutViewVK26MultiLang__Init(PCLayoutViewVK26MultiLang pltViewVK26)
{
	pltViewVK26->m_ltParent.m_ltParent.m_pfcPaint = CLayoutViewVK26MultiLang__Paint;
	return GBIME_OK;
}
...
GBIMELOCAL GBIMEReturn CLayoutViewVK26MultiLang__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	...
	CLASS_VIEW_THIS_POITER(CLayoutViewVK26MultiLang, pltThis)
	...
	if ((pltRoot->m_refreshFlag & GBIME_LAYOUT_REFRESH_CANDIDATE_LINE) == GBIME_LAYOUT_REFRESH_CANDIDATE_LINE)
	{
		...
		case FirstLayer:
			break;
		case SecondLayer:
			ICLayoutViewVK26MultiLang__DrawCandidateLine(pltObj, pEngineOutputInfo);
			break;
		case ThirdLayer:
			break;
		default:
...
*/
static void ICLayoutViewVK26MultiLang__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	CLASE_THIS_POITER(CLayoutVK26MultiLang, pltObj)
	CLASS_VIEW_THIS_POITER(CLayoutViewVK26MultiLang, pltThis)
	...
	// 打印Log信息
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************ICLayoutViewVK26MultiLang__DrawCandidate() Draw Candidate image\n"));
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************Candidate Image ID = %d\n", iCandidateNomalImageId));
	
	// 绘制输出串和候选字
	//pltViewBase->m_pfcDrawOutputString(pltObj, pEngineOutputInfo);[音节栏去掉，输入串当做首个候选]
