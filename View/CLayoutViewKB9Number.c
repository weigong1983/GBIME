...
GBIMELOCAL GBIMEReturn CLayoutViewKB9Number__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);

// 本地函数声明
GBIMELOCAL void ICLayoutViewKB9Number__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo);
...
CLayoutViewAbstract * CLayoutViewKB9Number__Construct(PCLayoutViewKB9Number pltViewKB9)
{
	...
	CLayoutViewKB9Number__Init(pltViewKB9);
	return (CLayoutViewAbstract *)pltViewKB9;
}

GBIMEReturn CLayoutViewKB9Number__Init(PCLayoutViewKB9Number pltViewKB9)
{
	pltViewKB9->m_ltParent.m_ltParent.m_pfcPaint = CLayoutViewKB9Number__Paint;
	return GBIME_OK;
}
...
GBIMELOCAL GBIMEReturn CLayoutViewKB9Number__Paint(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
 	...
 	CLASS_VIEW_THIS_POITER(CLayoutViewKB9Number, pltThis)
		...
		&& pEngineOutputInfo->bCandWindOpen)
	{
		...
		{
		case FirstLayer:
			ICLayoutViewKB9Number__DrawCandidateLine(pltObj, pEngineOutputInfo);
			break;
		default:
			PT_Assert(0);
			break;
...
static void ICLayoutViewKB9Number__DrawCandidateLine(GBLPVOID pltObj, PEngineOutputInfo pEngineOutputInfo)
{
	CLASE_THIS_POITER(CLayoutKB9Number, pltObj)
	CLASS_VIEW_THIS_POITER(CLayoutViewKB9Number, pltThis)
...
// 	PT_DrawImage(iDisplayImageLeftX ,iDisplayImageTopY,iCandidateNomalImageId);
	...
	// 打印Log信息
	PT_PrintLogInfo(GBLOG_LAYOUTVIEW, ("***********************ICLayoutViewKB9Number__DrawCandidateLine() Draw Candidate image\n"));
