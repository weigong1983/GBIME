...
extern "C" {
	 ...
	 * \date 2009年7月17日
	 */
	GBAPI GBRETURN GBInitialize(pGBInputStruct pGBIS, const GBLDB * pLDBs, GBINT nLDBCount);
	...
	*/
	 ...
	 */
	GBEXPORT GBRETURN GBSetInputString(pGBInputStruct pGBIS, GBLPCWCHAR pInputString);
	 ...
	 * \date 2010-7-21
	 */
	GBEXPORT GBRETURN GBHelperAddInputChar(pGBInputStruct pGBIS, GBWCHAR chr);
	 ...
	 * \date 2010-7-21
	 */
	GBEXPORT GBRETURN GBHelperAddInputString(pGBInputStruct pGBIS, GBLPCWCHAR pInputString);
	 ...
	 * \return 
		...
		- GBOK：成功返回
		- other：参考gbdef.h文件中的GBReturnEnum定义
	 * \note 后继调用GBGetCandidate 或者GBGetCandidateEx等获取候选的函数可以更新对应的候选列表
	...
	*/
	 ...
	 * \date 2009年7月16日
	 */
	GBAPI GBRETURN GBGetSyllables(pGBInputStruct pGBIS, GBINT nStartIndex, GBWCHAR ** ppSyllables,int nArrayLength);
	...
	* \date 2010年9月6日
	*/
	GBEXPORT GBRETURN GBGeSyllablePageCount(pGBInputStruct pGBIS,  GBINT * pPageCount);

	/*! \example GBGetCurrentSyllablePageIndex
	*/
	/*!
	* \brief 返回当前的音节页所有的音节页中的索引
	* \param pGBIS (in)pGBInputStruct对象
	...
	* \date 2010年9月6日
	*/
	GBEXPORT GBRETURN GBGetCurrentSyllablePageIndex(pGBInputStruct pGBIS, GBINT * pPageIndex);
	...
	* \date 2009年9月28日
	*/
	GBAPI GBRETURN GBWord2CodesByInputmod(pGBInputStruct pGBIS, int nInputmod, unsigned short isn,
	...
	* \date 2010年11月29日
	*/
	GBAPI GBRETURN GBSetSyllableTone(pGBInputStruct pGBIS, GBINT tone);
	/*@}*/
	...
	* \param pGIS (in)pGBInputStruct对象
	...
	* \date 2010-4-27
	*/
	GBEXPORT GBRETURN GBTransFormat(pGBInputStruct pGBIS, GBWCHAR * pCand);
...
#define GB_CURRENT_UDB_ID		(-1)
	...
	/*!
	 * \brief 设置中文 UDB 和 UpContent数据更新的策略
	 * \param pGBIS 
	 * \param nUDBID 有效的UDB ID
	 * \param nUpdateFreq 更新的频率，数据的修改次数大于这个值得时候，引擎就会调用fpUpdateFunction，一般来说这个值建议为10。
	 * \param fpUDBUpdateFunction 中文UDB更新的回调函数
	 * \param pOEMUDBUpdateParam OEM指定的一个参数，在回调函数里面回传给OEM移植代码
	 * \param fpUpContentUpdateFunction 中文UpContent更新的回调函数
	 * \param pUpContentOEMUpdateParam OEM指定的一个参数，在回调函数里面回传给OEM移植代码
	 * \return 
	 * \note 如果厂商只需要实现在退出的时候保存自造词，那么请使用\ref GBUDBGetModifiedCount来检查UDB是否被修改过
	 * \author chenbinghui
	 * \date 2009年7月28日
	 */
	GBEXPORT GBRETURN GBChnUDBSetUpdateParam(pGBInputStruct pGBIS, 
										  GBI32 nUDBID,
										  GBINT nUpdateFreq,
	 ...
	 * \date 2009年7月21日
	 */
	GBRETURN GBWriteLog1Param(pGBInputStruct pGBIS, 
		const char * file, 
		int line,	
		const char * msg, 
		GBU32 value);
	 ...
	 * \date 2009年7月23日
	 */
	typedef int (* fpLogRecorder_t)(GBLPVOID pOEMData, const char * format, unsigned long p1, unsigned long p2, unsigned long p3, unsigned long p4);
...
//GB Event API
	...
	* \date	2010年3月4日
	*/
	GBEXPORT GBRETURN GBMMIReset(pGBInputStruct pGBIS);
